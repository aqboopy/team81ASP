...
const sellValidationRules = () => {
	return [
		check("productname", "Please input the name of your product!").isLength(
			{ min: 1, max: 50 }
		),
		check("category", "Please select a category!").notEmpty(),
		check("price", "Please input a valid price!").notEmpty(),
		check("description", "Please describe your product!").isLength({
			min: 1,
			max: 100,
		}),
	];
...
router.post("/", upload.single("image"), sellValidationRules(), (req, res) => {
	...
	if (!errors.isEmpty()) {
		...
		return res.render("sell", { title: "Sell Product", alert });
	}

	if (!req.file) {
		return res.render("sell", {
			title: "Sell Product",
			alert: [{ msg: "Please provide a photo of your product!" }],
		});
	} else {
		...
		let insertQuery =
			"INSERT INTO products (name,category,price,description,image,image_type,date_listed,user_id) VALUES (?,?,?,?,?,?,CURRENT_TIMESTAMP,?)";
		global.db.run(
			insertQuery,
			[
				productname,
